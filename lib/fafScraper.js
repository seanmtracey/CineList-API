...
function getListingForCinemaByID(id, day){
	...
	day = day || 0;
	const cachedVersion = JSON.parse( cache.get(`${id}:${day}`) );

	if(cachedVersion){
		if(cachedVersion.date === new Date().getDay()){
			console.log("Using cached version");
			console.log(cachedVersion);
			delete cachedVersion.date;
	...
	return fetch(fafURL)
		...
		.then(data => {
			...
			const listingsToCache = listings;
			listingsToCache.date = new Date().getDay();
